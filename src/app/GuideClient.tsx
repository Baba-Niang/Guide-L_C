...
function ProductionCard({
  title, syntax, prototype, parameters, returns, rules, example,
}: {
  title: string; syntax: string; prototype?: string;
  parameters?: { name: string; desc: string }[];
  returns?: string; rules?: string[]; example?: string;
}) {
  return (
    <div className="production-card">
      <div className="pc-badge">RECETTE POUR PRODUIRE</div>
      <h3>{title}</h3>
      <div className="pc-label">Syntaxe</div>
      <pre className="pc-code"><code>{syntax}</code></pre>
      {prototype && <>
        <div className="pc-label">Prototype</div>
        <pre className="pc-code pc-code-soft"><code>{prototype}</code></pre>
      </>}
      {parameters && parameters.length > 0 && (
        <div className="pc-grid">
          {parameters.map((p) => <div className="pc-detail" key={p.name}><strong>{p.name}</strong><span>{p.desc}</span></div>)}
        </div>
      )}
      {returns && <div className="pc-return"><strong>Retour :</strong> {returns}</div>}
      {example && <>
        <div className="pc-label">Exemple complet</div>
        <pre className="pc-code pc-example"><code>{example}</code></pre>
      </>}
      {rules && rules.length > 0 && <ul className="pc-rules">{rules.map((r,i)=><li key={i}>{r}</li>)}</ul>}
    </div>
  );
}
  ...
  return (
    <div ref={scrollRef} className="h-full overflow-y-auto">
      ...
      <div className="content-wide mx-auto px-4 md:px-8 py-7">
        <div className="chapter-level-chip" aria-label={`Étape actuelle : ${LEVEL_META[currentLevel].num}`}>
          <span className="clc-num">{LEVEL_META[currentLevel].num}</span>
      ...
      {/* Bottom nav: moves through the 3 learning levels, then chapters */}
      <div className="chapter-foot-nav">
        <button onClick={() => {
          if (currentLevel === "lire") store.setLevel("comprendre");
          else if (currentLevel === "faire") store.setLevel("lire");
          else prevChapter();
        }} disabled={chapter.id <= 1 && currentLevel === "comprendre"}>
          <ChevronLeft size={17} /> <span>Précédent</span>
        </button>
        <div className="chapter-position">
          <span className="chapter-position-step">{currentLevel === "comprendre" ? "Je comprends" : currentLevel === "lire" ? "Je sais lire" : "Je sais faire"}</span>
          <span className="chapter-position-count">{chapter.id} / {TOTAL_CHAPTERS}</span>
        </div>
        <button onClick={() => {
          if (currentLevel === "comprendre") store.setLevel("lire");
          else if (currentLevel === "lire") store.setLevel("faire");
          else nextChapter();
        }} disabled={chapter.id >= TOTAL_CHAPTERS && currentLevel === "faire"}>
          <span>Suivant</span> <ChevronRight size={17} />
        </button>
      </div>
    </div>
  );
      ...
      <header className="app-topbar">
        <div className="topbar-profile">
          <button onClick={() => setView("home")} className="topbar-avatar" aria-label="Accueil">
            <img src="./baba-niang.jpg" alt="Baba Niang" />
          </button>
